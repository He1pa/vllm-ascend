...
template <typename T, GroupMultiFlag GROUP_FLAG = GroupMultiFlag::UNDEFINED>
class GroupTopk
{
public:
    ...
    // init
    __aicore__ explicit GroupTopk(
        __gm__ void *topKInput, 
        __gm__ void *idxArr, 
        AscendC::TPipe &pipe,
        ...
        : blockIdx(AscendC::GetBlockIdx()), groupNum(groupNum),
          groupPadded(RoundUp(groupNum, SORT_REPEAT_COUNT)), k(k), kInner(kInner),
          expertNumPerGroup(expertNumPerGroup), expertNumPerGroupPadded(expertNumPerGroupPadded),
          expertNumPerToken(expertNumPerToken),
          tokenNum(tokenNumPerCore + static_cast<uint32_t>(tailTokenNum > blockIdx)),
          tokenOffSet(tokenNumPerCore * blockIdx +
                      (tailTokenNum > blockIdx ? blockIdx : tailTokenNum)),
          ...
          padParams(true, leftPadding, rightPadding, T_MIN)
    {
        ...
        pipe.InitBuffer(buf, BUF_SIZE); // BUF_SIZE = 180 * 1024;
        ...
        // idxArrGm 是用户传入的全局内存数组，存储原始专家或分组的索引（如 [0, 1, 2, ..., MAX_EXPERT-1]）
        // indexTensor 是本地缓冲区，初始化时从 idxArrGm 拷贝数据，确保后续操作基于原始索引。
    ...
    __aicore__ inline void Process()
    {
        ...
        {
            ProcessSmallGroupNum(); // 小分组或基础模式
        }
        else
        {
            ProcessLargeGroupNum(); // 大分组优化模式
        }
    }
    ...
    __aicore__ inline void ProcessSmallGroupNum()
    {
        ...
        CopyIn(0, event0); // 加载第0个token到数据缓冲区0
        AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(event0); // 标记拷贝完成
        AscendC::WaitFlag<AscendC::HardEvent::MTE2_V>(event0); // 等待拷贝完成
        Topk(event0);
        ...
        {
            AscendC::WaitFlag<AscendC::HardEvent::MTE3_MTE2>(event1); // 等待前次回写完成
            CopyIn(i + 1, event1); // 加载下一个Token到缓冲区1
            AscendC::SetFlag<AscendC::HardEvent::MTE2_V>(event1); // 标记新数据拷贝完成
            SetZeroGroup(event0); // 清零
            AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(event0); //标记清零
            AscendC::WaitFlag<AscendC::HardEvent::V_MTE3>(event0); // 等待清零
            CopyOut(i, event0); // 会写结果到全局内存
    ...
    __aicore__ inline void SetZeroGroup(event_t event)
    {
        ...
        // 使用 Brcb 和 GatherMask 操作生成块级选择掩码
    ...
    // 根据排序结果设置组选择掩码 groupSelectTensor
 ...
 * elements slowly.
 */
// 特殊优化场景 单元素分组优化（SingleValueGroupTopk）
template <typename T>
class SingleValueGroupTopk
{
public:
    static constexpr T T_MIN = std::is_same_v<T, half> ? -65504 : -3.38953e+38;
    using CalcType = std::conditional_t<std::is_same_v<T, bfloat16_t>, float, half>;

    __aicore__ explicit SingleValueGroupTopk(       
        __gm__ void *topKInput, 
        __gm__ void *idxArr, 
        AscendC::TPipe &pipe,
        const uint32_t groupNum, 
        ...
        const uint32_t tokenNumPerCore, 
        const uint32_t tailTokenNum)
        : blockIdx(AscendC::GetBlockIdx()), k(k), expertNumPerToken(expertNumPerToken),
          tokenNum(tokenNumPerCore + static_cast<uint32_t>(tailTokenNum > blockIdx)),
          tokenOffSet(tokenNumPerCore * blockIdx +
                      (tailTokenNum > blockIdx ? blockIdx : tailTokenNum)),
...
__aicore__ inline void group_topk_impl(
    __gm__ void *topKInput, 
    __gm__ void *idxArr,     
    const uint32_t groupNum, 
    const uint32_t k, 
...
{
    AscendC::TPipe pipe;
    GroupTopk<T, GROUP_FLAG> op(topKInput, idxArr, pipe, groupNum, k, kInner, expertNumPerGroup, expertNumPerGroupPadded, expertNumPerToken, tokenNumPerCore, tailTokenNum);
    op.Process();
    return;
}

template <typename T>
__aicore__ inline void single_value_group_topk_impl(
    __gm__ void *topKInput, 
    __gm__ void *idxArr,     
    const uint32_t groupNum, 
    const uint32_t k, 
...
{
    AscendC::TPipe pipe;
    SingleValueGroupTopk<T> op(topKInput, idxArr, pipe, groupNum, k, kInner, expertNumPerGroup, expertNumPerGroupPadded, expertNumPerToken, tokenNumPerCore, tailTokenNum);
    op.Process();
    return;
}

extern "C" __global__ __aicore__ void group_topk(
    __gm__ void *topKInput, 
    __gm__ void *idxArr,
    const uint32_t groupNum, 
    const uint32_t k, 
...
{
    // event0 = event1; event1 = event2; event2 = eventTemp; 会出现 use of undeclared identifier 'matmul'
    // https://www.hiascend.com/forum/thread-02104168004092996067-1-1.html
    KERNEL_TASK_TYPE_DEFAULT(KERNEL_TYPE_AIV_ONLY);

    if (tilingKey==20000000000)
    {
        group_topk_impl<half, GroupMultiFlag::UNDEFINED>(topKInput, idxArr, groupNum, k, kInner, expertNumPerGroup, expertNumPerGroupPadded, expertNumPerToken, tokenNumPerCore, tailTokenNum);
    }
    else if (tilingKey == 20000000010)
    {
        group_topk_impl<half, GroupMultiFlag::SUM_MULTI_MAX>(topKInput, idxArr, groupNum, k, kInner, expertNumPerGroup, expertNumPerGroupPadded, expertNumPerToken, tokenNumPerCore, tailTokenNum);
    }
    else if (tilingKey == 900)
    {
        single_value_group_topk_impl<half>(topKInput, idxArr, groupNum, k, kInner, expertNumPerGroup, expertNumPerGroupPadded, expertNumPerToken, tokenNumPerCore, tailTokenNum);
    }

#if defined(__CCE_KT_TEST__) || (__CCE_AICORE__ == 220)
    else if (tilingKey == 20000000001)
    {
        group_topk_impl<bfloat16_t, GroupMultiFlag::UNDEFINED>(topKInput, idxArr, groupNum, k, kInner, expertNumPerGroup, expertNumPerGroupPadded, expertNumPerToken,  tokenNumPerCore, tailTokenNum);
    }
    else if (tilingKey == 20000000011)
    {
        group_topk_impl<bfloat16_t, GroupMultiFlag::SUM_MULTI_MAX>(topKInput, idxArr, groupNum, k, kInner, expertNumPerGroup, expertNumPerGroupPadded, expertNumPerToken,  tokenNumPerCore, tailTokenNum);
    }
    else if (tilingKey == 901)
    {
        single_value_group_topk_impl<bfloat16_t>(topKInput, idxArr, groupNum, k, kInner, expertNumPerGroup, expertNumPerGroupPadded, expertNumPerToken,  tokenNumPerCore, tailTokenNum);
    }
#endif
}

namespace vllm_ascend {
    void group_topk_impl(
        void* topKInput,
        void* idxArr,
        const uint32_t groupNum, 
        const uint32_t k, 
    ...
    {
        group_topk<<<aiv_num, nullptr, stream>>>(
            topKInput,
            idxArr,
            groupNum,
            k,
